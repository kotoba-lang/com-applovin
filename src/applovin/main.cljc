(ns applovin.main
  "Kotodama WASM entrypoint for the Applovin clean-room actor (L4) — Clojure port.
  ...
  facts (`applovin.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "applovin")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"applovin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "applovin-compat" :tier tier :entities entities} 200])
...
(defn start! [] :applovin-compat/ready)
